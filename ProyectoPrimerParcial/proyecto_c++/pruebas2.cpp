...
void weigths(int **W,  int *nat, int *murso, int *boluz, int *phantom, 
            int *gremio, int N)
{
  ...
  for (int j=0; j<N; j++)
  {
    W[i][j] =  nat[i]*nat[j] + murso[i]*murso[j] + boluz[i]*boluz[j] + 
    phantom[i]*phantom[j] + gremio[i]*gremio[j];
  }
  for (int k=0; k<N; k++)
    W[k][k]=0;
...
int readfile(string file, int *v1, int col, int N)
{
    ...
    while(infile >> a1 >> a2 >> a3 >> a4 >> a5 >> a6 >> a7 >> a8 >> a9 >> a10
           >> a11 >> a12) {
        std::cout << a1 << a2 << a3 << a4 << a5 << a6 << a7 << a8 << a9 
         << a10 << a11 << a12 << endl;
        v1[(row*col) + 0] = a1;
        v1[(row*col) + 1] = a2;
        v1[(row*col) + 2] = a3;
...
int main(void)
{
  ...
  int *murso = new int[N];
  int *boluz = new int[N];
  int *phantom = new int[N];
  int *gremio = new int[N];  


  cout << "iterations " << endl;

  readfile("nat.txt", nat,  col, N);
  readfile("murso.txt", murso,  col, N);
  readfile("boluz.txt", boluz,  col, N);
  readfile("phantom.txt", phantom,  col, N);
  readfile("gremio.txt", gremio, col, N);
  // pattern 0
/*  x0[0] = 1;
  x0[1] = 1;
  x0[2] = 1;
...
*/
  ...
  weigths(W,  nat, murso, boluz,phantom,gremio,N);
  printw(W, N);
